"""
Sudoku Solver 
    Project for my Artificial Intelligence class at UFMG in 2018.
...
def ImportSudoku(n,level,id):
    sudoku=[]
    name = (".\sudoku_unsolved\Sudoku_%ix%i_%s_%i - Sheet1.csv" % (n,n,level,id))
    f = open(name,'r')
    for line in f:
        sudoku_line = []
...
def ImportSudoku2(filename,n=9):
    sudoku=[]
    name = (filename)
    f = open(name,'r')
    for line in f:
...
def CreateSudokuFromText(text,n=9):
    sudoku=[]
    f = text
    for line in f:
        sudoku_line = []
...
def FormatSudoku(sudoku_unsolved,n=9):
    sudoku=[]
    for line in sudoku_unsolved:
        sudoku_line = []
        for x in list(line):
            sudoku_line.append(str(x))
        sudoku.append(sudoku_line)
        np.array(sudoku_line, dtype=object)
    n = int(len(sudoku))
    sudoku = np.array(sudoku, dtype=object)
    sudoku.reshape((n, n))
    return sudoku
...
def AC_3_and_Heuristics(csp,a=3):
    ni = n_domain_complete(csp)
    print('AC_3 ', "Initial: ", ni)
    while 1:
        print("")

        #inicio = time.clock() # deprecated
        inicio=time.process_time()
        flag_AC_3=AC_3(csp)
        if(flag_AC_3) == False: return False
        n = n_domain_complete(csp)
        print('AC_3 ', flag_AC_3, n, time.process_time()-inicio)
        if(n[1]==0):
            return True
        ...
        print('NSO ', flag_NumeroSozinhoOculto, n2, time.process_time()-inicio)
        if(n2[1]==0):
            return True
        ...
        print('PS', flag_ParesSozinhos, n3, time.process_time()-inicio)
        if(n3[1]==0):
            return True
        ...
        print('SE', flag_SubsetEscondido, n4, time.process_time()-inicio)
        if(n4[1]==0):
            return True
        ...
        if n3 == n and flag_ParesSozinhos == False:
            ...
            print('BCRI', flag_Block_Column_Row_Interaction, n5, time.process_time()-inicio)
...
if __name__=="__main__":
    ...
    "AC_3 + Heuristics + Backtracking"
    print(AC_3_and_Heuristics(csp,a))
    ...
    print("Time:", time.process_time()-inicio)
